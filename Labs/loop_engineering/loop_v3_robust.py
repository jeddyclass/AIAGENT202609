...
# ==========================================================
def run_code_repair_loop(max_iterations: int = 10):
    system_prompt = (
        "You are an expert Python Engineer.\n"
        "Your task is to fix a TTLCache class to pass all unit tests.\n"
        "Rules:\n"
        "1. Do not break basic operations while fixing edge cases.\n"
        "2. Ensure get() marks items as recently used (move_to_end).\n"
        "3. Ensure expired items are removed and not counted in size().\n"
        "4. Return ONLY valid, executable Python code with no markdown formatting or text."
    )
    ...
    for attempt in range(1, max_iterations + 1):
        print(f"{'='*28} [Loop 迭代 {attempt}/{max_iterations}] {'='*28}")
